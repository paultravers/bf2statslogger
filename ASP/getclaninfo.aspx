...
if (!is_numeric($listtype)) {
	...
	switch ($listtype) {
		...
		case 1:		#Whitelist
			if ($_GET['clantag']) {
				...
				$where .= " AND LEFT(`name`, {$paramLen}) = '" . quote_smart($_GET['clantag']) . "'  AND `permban` = 0";
			}
			break;
		case 2:		#Greylist
			// Get Criteria
				...
				if ($_GET[$param]) {
					switch ($param) {
						...
						case 'kdratio':
							...
							break;
						case 'country':
							$paramArray = str_replace (",", "','", quote_smart($_GET[$param]));
	...
	@mysql_select_db($cfg->get('db_name'), $connection);
		...
		$query = "SELECT id, name FROM player WHERE ip <> '127.0.0.1'" . $where . " ORDER BY id ASC";
