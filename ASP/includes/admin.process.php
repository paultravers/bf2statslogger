		...
		while( $rowsrc = mysql_fetch_array( $result ) ) {
			...
			if ($rowsrc['mapid']>=0) {
				...
				if( mysql_num_rows( $chkresult ) ) {
					...
					$query .= "`time` = `time` + " . $rowdest['time'] . ",\n";
					$query .= "`win` = `win` + " . $rowdest['win'] . ",\n";
					$query .= "`loss` = `loss` + " . $rowdest['loss'] . ",\n";
					$query .= "`best` = `best` + " . $rowdest['best'] . ",\n";
					$query .= "`worst` = `worst` + " . $rowdest['worst'] . "\n";
...
function processCheckAwards() {
	...
	if (!isset($backendawardsdata)) {
		$backendawardsdata = buildBackendAwardsData('bf2');
	}
